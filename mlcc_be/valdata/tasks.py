...
class model_controller:
    def __init__(self) -> None:
        ...
        self.running = False
        self.results = []
        self.model_root = "C:/Users/user/Desktop/IITP/mmcv_laminate_alignment_system"
        self.dt = datetime.now().strftime('%y%m%d_%H%M%S')

    @classmethod
    @shared_task
    def get_model_output(self) -> None:
        if self.mode == 'auto':
            self.auto_get_result()
        elif self.mode == 'manual':
            self.manual_get_result()

    
    def auto_get_result(self) -> None:
        if self.running:
            return -1
        self.running = True
        try:
            # 1. 모델 실행
            dir_path = f"{self.model_root}/mlcc_datasets/val_test"
            ...
            if length != 0:
                # 2. 데이터 적재
                self.results = auto_run_model(self.dt) 
                for i, result in enumerate(self.results):
                    self.save_result(i)

            # 3. DB 적재한 모델 원본 데이터 삭제
            entries = os.scandir(f'{self.model_root}/mlcc_datasets/val_test')
            for entry in entries:
    ...
    def manual_get_result(self) -> None:
        if self.running:
            return -1
        self.running = True
        try:   
            # 실행 경로 정하기
            dir_path = f"{self.model_root}/mlcc_datasets/smb"
            ...
            if pc_name != '':
                ...
                os.makedirs(f'{dir_path}/{pc_name}/{self.dt}')
                for result in results:
                    ...
                    f = open(f'{dir_path}/{pc_name}/{self.dt}/{result["img_basename"]}_{str(assessment)}.txt', 'w')
                    f.close()
                    shutil.copyfile(f'{dir_path}/{pc_name}/{result["img_basename"]}', f'{backup_path}/{self.dt}_{result["img_basename"]}')
                    shutil.move(f'{dir_path}/{pc_name}/{result["img_basename"]}', f'{dir_path}/{pc_name}/{self.dt}/{result["img_basename"]}')
                    # 3. DB에 Log 적재
                    log = ManualLog()
